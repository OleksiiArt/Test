...
@WebServlet(urlPatterns = { "/crewMembers" })
public class CrewMembersServlet extends HttpServlet{
	
	
	private static final long serialVersionUID = 9113914193583628200L;

	public CrewMembersServlet() {
	 ...
	 protected void doGet(HttpServletRequest request, HttpServletResponse response)
	        ...
	        try {
      list = DBManager.getInstance().findWorker(con);
	        } catch (Exception e) {
	            e.printStackTrace();
	        ...
	        RequestDispatcher dispatcher = request.getServletContext()
	                .getRequestDispatcher("/WEB-INF/views/admin/crewMembers.jsp");
